...
class GostAdapter:
    ...
    def apply(self, tunnel_id: str, spec: Dict[str, Any]):
        ...
        sni_hostname = spec.get('sni_hostname') or spec.get('stealth_sni')
        ...
        for port in ports:
            ...
            if tunnel_type == "tcp":
                ...
                cmd.append(f"-L=grpc://{listen_addr}/{target_addr}{opt}")
            elif tunnel_type == "tcpmux":
                cmd.append(f"-L=tcpmux://{listen_addr}/{target_addr}")
            elif tunnel_type == "quic":
                opt = f"?host={sni_hostname}" if sni_hostname else ""
                cmd.append(f"-L=quic://{listen_addr}/{target_addr}{opt}")
            elif tunnel_type == "relaytls":
                opt = f"?host={sni_hostname}" if sni_hostname else ""
                cmd.append(f"-L=relay+tls://{listen_addr}/{target_addr}{opt}")
            else:
