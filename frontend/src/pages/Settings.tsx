...
const Settings = () => {
  ...
  const exportTunnels = async () => {
    setBackupMessage(null)
    try {
      const res = await api.get('/tunnels/export')
      const blob = new Blob([JSON.stringify(res.data, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `iranfree-tunnels-${new Date().toISOString().slice(0, 10)}.json`
      a.click()
      URL.revokeObjectURL(url)
      setBackupMessage('Tunnels exported successfully.')
    } catch (e: any) {
      setBackupMessage(e.response?.data?.detail || 'Export failed.')
    }
  }

  const importTunnels = async (e: React.ChangeEvent<HTMLInputElement>) => {
    setBackupMessage(null)
    ...
    try {
      ...
      const tunnels = data.tunnels || (Array.isArray(data) ? data : [])
      if (tunnels.length === 0) {
        setBackupMessage('No tunnels in file.')
        return
      }
      await api.post('/tunnels/import', { tunnels })
      setBackupMessage(`Imported ${tunnels.length} tunnel(s). You can activate them from the Tunnels page.`)
    } catch (err: any) {
      setBackupMessage(err.response?.data?.detail || err.message || 'Import failed.')
    }
    e.target.value = ''
    ...
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Export all tunnels as JSON for backup or migration. Import on a new server to restore tunnel definitions (node IDs may need to be re-assigned).
          </p>
          {backupMessage && (
            <p className="text-sm mb-4 text-green-600 dark:text-green-400">{backupMessage}</p>
          )}
          <div className="flex flex-wrap gap-4">
            <button
              type="button"
              onClick={exportTunnels}
