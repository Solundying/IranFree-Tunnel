...
const EditTunnelModal = ({ tunnel, onClose, onSuccess }: EditTunnelModalProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    remote_ip: remoteIp,
    sni_hostname: tunnel.spec?.sni_hostname || tunnel.spec?.stealth_sni || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (tunnel.core === 'gost' && (tunnel.type === 'tcp' || tunnel.type === 'udp' || tunnel.type === 'grpc' || tunnel.type === 'tcpmux' || tunnel.type === 'quic' || tunnel.type === 'relaytls')) {
        const remoteIp = formData.remote_ip || '127.0.0.1'
        updatedSpec.remote_ip = remoteIp
        updatedSpec.ports = ports
        updatedSpec.remote_port = ports[0]
        updatedSpec.listen_port = ports[0]
        if (formData.sni_hostname?.trim()) updatedSpec.sni_hostname = formData.sni_hostname.trim()
        else delete updatedSpec.sni_hostname
      } else if (tunnel.core === 'chisel') {
        updatedSpec.ports = ports
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100]">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {tunnel.core === 'gost' && (tunnel.type === 'tcp' || tunnel.type === 'udp' || tunnel.type === 'grpc' || tunnel.type === 'tcpmux' || tunnel.type === 'quic' || tunnel.type === 'relaytls') && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t.tunnels.remoteIP}
              ...
              {(tunnel.type === 'grpc' || tunnel.type === 'quic' || tunnel.type === 'relaytls') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Stealth SNI / Hostname (optional)
                  </label>
                  <input
                    type="text"
                    value={formData.sni_hostname}
                    onChange={(e) => setFormData({ ...formData, sni_hostname: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white"
...
const AddTunnelModal = ({ nodes, servers, onClose, onSuccess }: AddTunnelModalProps) => {
  ...
  const [formData, setFormData] = useState({
    name: '',
    core: 'gost',
    type: 'grpc',  // default: gRPC (censorship-resistant)
    node_id: '',
    foreign_node_id: '',
    iran_node_id: '',
    ports: '8080',  // Comma-separated ports (e.g., "8080,8081,8082")
    remote_ip: '127.0.0.1',
    sni_hostname: '',  // Stealth SNI/hostname (e.g. google.com) for TLS/gRPC/QUIC - use same in client
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.core === 'gost' && (formData.type === 'tcp' || formData.type === 'udp' || formData.type === 'grpc' || formData.type === 'tcpmux' || formData.type === 'quic' || formData.type === 'relaytls')) {
        ...
        if (formData.sni_hostname && formData.sni_hostname.trim()) {
          spec.sni_hostname = formData.sni_hostname.trim()
        }
      }
  ...
  const getSpecForType = (core: string, type: string): Record<string, any> => {
    ...
    switch (type) {
      ...
      case 'quic':
      case 'relaytls':
        return { ...baseSpec, service_name: type === 'grpc' ? 'GrpcService' : type, uuid: generateUUID() }
      case 'udp':
  ...
  const handleCoreChange = (core: string) => {
    ...
    if (core === 'rathole' || core === 'chisel') {
      ...
      newType = (formData.type === 'tcp' || formData.type === 'udp' || formData.type === 'grpc' || formData.type === 'quic' || formData.type === 'relaytls' || formData.type === 'tcpmux') ? formData.type : 'tcp'
    } else if (core === 'backhaul') {
      newType = backhaulState.transport
    } else if (core === 'gost') {
      const gostTypes = ['tcp', 'udp', 'ws', 'grpc', 'tcpmux', 'quic', 'relaytls']
      newType = gostTypes.includes(formData.type) ? formData.type : 'grpc'
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] overflow-auto">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 w-full max-w-xl my-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {formData.core === 'chisel' ? (
                  ...
                  <>
                    ...
                    <option value="relaytls">Relay+TLS (recommended)</option>
          ...
          {formData.core === 'gost' && (formData.type === 'tcp' || formData.type === 'udp' || formData.type === 'grpc' || formData.type === 'tcpmux' || formData.type === 'quic' || formData.type === 'relaytls') && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t.tunnels.remoteIP}
          ...
          {formData.core === 'gost' && (formData.type === 'grpc' || formData.type === 'quic' || formData.type === 'relaytls') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Stealth SNI / Hostname (optional)
              </label>
              <input
                ...
                value={formData.sni_hostname}
                onChange={(e) =>
                  setFormData({ ...formData, sni_hostname: e.target.value })
                }
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white"
                placeholder="e.g. google.com or microsoft.com"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Fake domain for TLS/gRPC/QUIC so DPI sees this hostname. Use the same value in your client config.
              </p>
            </div>
          )}
