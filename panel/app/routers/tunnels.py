...
async def create_tunnel(tunnel: TunnelCreate, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        needs_gost_forwarding = db_tunnel.type in ["tcp", "udp", "ws", "grpc", "tcpmux", "quic", "relaytls"] and db_tunnel.core == "gost" and not is_reverse_tunnel
                    ...
                    if ports and hasattr(request.app.state, 'gost_forwarder'):
                        try:
                            for port in ports:
                                ...
                                sni_hostname = (db_tunnel.spec or {}).get("sni_hostname") or (db_tunnel.spec or {}).get("stealth_sni")
...
async def update_tunnel(
    ...
    if spec_changed:
        try:
            needs_gost_forwarding = tunnel.type in ["tcp", "udp", "ws", "grpc", "tcpmux", "quic", "relaytls"] and tunnel.core == "gost"
            ...
            if needs_gost_forwarding:
                ...
                if panel_port and forward_to and hasattr(request.app.state, 'gost_forwarder'):
                    try:
                        ...
                        sni_hostname = (tunnel.spec or {}).get("sni_hostname") or (tunnel.spec or {}).get("stealth_sni")
                        request.app.state.gost_forwarder.start_forward(
                            tunnel_id=tunnel.id,
...
async def export_tunnels(db: AsyncSession = Depends(get_db)):
    """Export all tunnels as JSON for backup or migration."""
    result = await db.execute(select(Tunnel).order_by(Tunnel.created_at))
    tunnels = result.scalars().all()
    payload = []
    for t in tunnels:
        payload.append({
    ...
    from datetime import datetime
    return ExportResponse(tunnels=payload, exported_at=datetime.utcnow().isoformat() + "Z")
...
@router.post("/import")
async def import_tunnels(req: ImportRequest, db: AsyncSession = Depends(get_db)):
    """Import tunnels from JSON backup. New IDs are generated; nodes must exist or be re-assigned later."""
    created = 0
    for item in req.tunnels:
        name = item.get("name") or "imported"
        core = item.get("core") or "gost"
        type_ = item.get("type") or "tcp"
        node_id = item.get("node_id") or ""
        foreign_node_id = item.get("foreign_node_id")
        iran_node_id = item.get("iran_node_id")
        ...
        created += 1
    await db.commit()
    return {"status": "ok", "message": f"Imported {created} tunnel(s). Set status to active and assign nodes if needed."}
