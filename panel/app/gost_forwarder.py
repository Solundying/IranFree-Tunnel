"""Gost v3 forwarding service: TCP/UDP/WS/gRPC/Relay+TLS/QUIC tunnels (IranFree Tunnel)"""
import subprocess
import time
import logging
import os
...
def _resolve_gost_binary() -> str:
    """Resolve Gost v3 binary: host-installed (Iran bootstrap) or PATH."""
    for candidate in ["/usr/local/bin/gost", "/opt/iranfree-gost/gost", "gost"]:
        if candidate == "gost":
            path = shutil.which("gost")
            if path:
                return path
            break
        if os.path.exists(candidate) and os.access(candidate, os.X_OK):
            return candidate
    raise RuntimeError("gost binary not found. Install Gost v3 (e.g. run panel install script with Iran/bootstrap).")


class GostForwarder:
    """Manages TCP/UDP/WS/gRPC/Relay+TLS/QUIC forwarding using Gost v3"""
    ...
    def start_forward(self, tunnel_id: str, local_port: int, forward_to: str, tunnel_type: str = "tcp", path: str = None, use_ipv6: bool = False, sni_hostname: Optional[str] = None) -> bool:
        """
        Start forwarding using Gost v3 - forwards directly to target (no node).
        ...
        Args:
            ...
            tunnel_type: tcp, udp, ws, grpc, tcpmux, quic, relaytls (gRPC/Relay+TLS/QUIC recommended for censorship resistance)
            ...
            sni_hostname: Optional stealth SNI/hostname (e.g. google.com) for TLS/gRPC/QUIC to evade DPI; use same value in client config
        """
        try:
            ...
            gost_binary = _resolve_gost_binary()
            # Gost v3: -L protocol://listen/target (same as v2 for basic forward)
            if tunnel_type == "tcp":
                cmd = [gost_binary, f"-L=tcp://{listen_addr}/{target_addr}"]
            elif tunnel_type == "udp":
                cmd = [gost_binary, f"-L=udp://{listen_addr}/{target_addr}"]
            elif tunnel_type == "ws":
                import socket
                try:
                    if use_ipv6:
                ...
                cmd = [gost_binary, f"-L=ws://{bind_ip}:{local_port}/tcp://{target_addr}"]
            elif tunnel_type == "grpc":
                opt = f"?host={sni_hostname}" if sni_hostname else ""
                cmd = [gost_binary, f"-L=grpc://{listen_addr}/{target_addr}{opt}"]
            elif tunnel_type == "tcpmux":
                cmd = [gost_binary, f"-L=tcpmux://{listen_addr}/{target_addr}"]
            elif tunnel_type == "quic":
                opt = f"?host={sni_hostname}" if sni_hostname else ""
                cmd = [gost_binary, f"-L=quic://{listen_addr}/{target_addr}{opt}"]
            elif tunnel_type == "relaytls":
                opt = f"?host={sni_hostname}" if sni_hostname else ""
                cmd = [gost_binary, f"-L=relay+tls://{listen_addr}/{target_addr}{opt}"]
            else:
        ...
        if not is_alive and tunnel_id in self.forward_configs:
            ...
            try:
                ...
                self.start_forward(
                    ...
                    sni_hostname=config.get("sni_hostname"),
                )
                return True
            except Exception as e:
