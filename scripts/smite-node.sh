#!/bin/bash
# IranFree Tunnel - Node Installer
...
echo "=== IranFree Tunnel - Node Installer ==="
echo ""
...
if [ -d "$INSTALL_DIR" ] && [ -f "$INSTALL_DIR/docker-compose.yml" ]; then
    echo "IranFree Tunnel Node already installed in $INSTALL_DIR"
    cd "$INSTALL_DIR"
else
    echo "Setting up IranFree Tunnel Node..."
    mkdir -p "$INSTALL_DIR"
    cd "$INSTALL_DIR"
...
# Enable BBR and optionally BBR v2/v3 (bbr2)
...
if modprobe -n tcp_bbr2 2>/dev/null; then
    if ! grep -q "tcp_bbr2" /etc/modules-load.d/*.conf 2>/dev/null; then
        ...
        echo "✅ BBR v2/v3 (bbr2) enabled if supported"
    fi
fi
...
if docker ps | grep -q smite-node; then
    echo ""
    echo "✅ IranFree Tunnel Node installed successfully!"
    echo ""
